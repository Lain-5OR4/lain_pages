...
**2026-07-07 以降**: wrangler.jsonc で `"workers_dev": false` / `"preview_urls": false` を設定済み。公開ホストは `api.mizora.dev` の1つだけなので、Access Application もこのホストの `admin*` に対してだけ存在すればよい。workers.dev 経由の裏口は存在しない(デプロイ時に無効化される)。
