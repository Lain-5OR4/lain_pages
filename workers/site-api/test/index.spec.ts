...
describe("photo-diary read path", () => {
  it("GET / lists posts in posted_on DESC order with image URLs", async () => {
    ...
    expect(body.indexOf("newer carousel post")).toBeLessThan(body.indexOf("old post"));
  ...
  it("GET /images/:key serves bytes from R2 with immutable cache", async () => {
    const res = await SELF.fetch("http://example.com/images/posts/1/0-aaa.jpg");
    expect(res.status).toBe(200);
...
describe("photo-diary /api/diary + CORS", () => {
  it("GET /api/diary returns DiaryEntry[] with absolute photo URLs and stamps", async () => {
    ...
    expect(e1?.photos[0].src).toBe("http://example.com/images/posts/1/0-aaa.jpg");
  ...
  it("GET /api/diary shifts taken_at with UTC marker to JST in the stamp", async () => {
    ...
    const entries = (await (await SELF.fetch("http://example.com/api/diary")).json()) as Array<{
      id: string;
      photos: Array<{ stamp?: string }>;
    }>;
...
describe("photo-diary write path", () => {
  ...
  it("DELETE /admin/posts/:id removes the post and its R2 objects", async () => {
    ...
    const res = await SELF.fetch(`http://example.com/admin/posts/${created.id}`, {
      method: "DELETE",
    });
    expect(res.status).toBe(200);
    ...
    const row = await env.DB.prepare("SELECT id FROM posts WHERE id = ?").bind(created.id).first();
    expect(row).toBeNull();
  });
  ...
  it("POST /admin/posts/:id/delete (form fallback) redirects to /admin", async () => {
    ...
    const res = await SELF.fetch(`http://example.com/admin/posts/${created.id}/delete`, {
      method: "POST",
      redirect: "manual",
    });
