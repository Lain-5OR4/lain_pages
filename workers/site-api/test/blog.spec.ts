...
describe("blog proxy /api/blog", () => {
  it("GET /api/blog returns the microCMS list with cache + CORS headers", async () => {
    fetchMock
      ...
      .reply(200, {
        contents: [{ id: "post-a", title: "A", publishedAt: "2026-06-01T00:00:00.000Z" }],
      });
  ...
  it("GET /api/blog/:id?draftKey=xxx forwards the key and disables caching", async () => {
    fetchMock
      .get(MICROCMS_ORIGIN)
      .intercept({
        path: (p) => p.startsWith("/api/v1/blogs/draft-post") && p.includes("draftKey=secret"),
      })
      .reply(200, { id: "draft-post", title: "Draft" });

    const res = await SELF.fetch("http://example.com/api/blog/draft-post?draftKey=secret");
    expect(res.status).toBe(200);
