...
- [x] **コード側の命名整理** — `workers/site-api/`、package名 `site-api` に変更。写真日記・読書記録・blog中継・管理画面を担当するサイトAPIとして扱う (2026-09-10)。
- [ ] **Cloudflare Worker名の移行** — 将来の名前は `mizora-api`。現行Worker名 `photo-diary`、D1 `photo-diary-db`、R2 `photo-diary-photos`、公開URL `api.mizora.dev` は維持。コードの改名をpushする前にWorkers Buildsのルートディレクトリを `workers/site-api` に合わせる。移行計画は `workers/site-api/README.md` を参照。
